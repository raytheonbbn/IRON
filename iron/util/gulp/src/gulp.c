...
void *Reader(void *arg)
	...
	if (tmpfd >= 0) {
          if (read(tmpfd, (char *)&fh, sizeof(fh)) < 0) {
	...
	if (pcs.ps_drop > 0) {
	    ...
	    if (procf) {
              if (fscanf(procf, "%d", &rmem_def) == EOF) {
                fprintf(stderr, "%s: fscanf error\n", progname);
                exit(EXIT_FAILURE);
              }
              fclose(procf);
            }
	    ...
	    if (procf) {
              if (fscanf(procf, "%d", &rmem_max) == EOF) {
                fprintf(stderr, "%s: fscanf error\n", progname);
                exit(EXIT_FAILURE);
              }
              fclose(procf);
...
newoutfile(char *dir, int num) {
    ...
    if (fchown(tmpfd, getuid(), -1) == -1) {
      fprintf(stderr, "%s: fchown error\n", progname);
      return 0;
    }
    if (tmpfd >= 0) {
...
void *Writer(void *arg)
	...
	if (seteuid(getuid()) == -1) exit(1);		/* drop setuid privilege */
	}
