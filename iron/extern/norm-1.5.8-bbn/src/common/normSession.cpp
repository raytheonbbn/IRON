...
NormSession::NormSession(NormSessionMgr& sessionMgr, NormNodeId localNodeId) 
 : session_mgr(sessionMgr), notify_pending(false), tx_port(0), tx_port_reuse(false),
   ...
   user_data(NULL), fc(NULL), ssm_src_port(0), next(NULL)
{
    interface_name[0] = '\0';
...
bool NormSession::Open()
{
    ...
    if (address.IsMulticast())
    {
        ...
        if (ssm_src_port != 0)
        {
          fprintf(stderr, "Calling rx_socket.SetSsmSrcPort from "
                  "NormSession::Open()\n");
          rx_socket.SetSsmSrcPort(ssm_src_port);
        }
    }
...
{   
    ...
    if (msgType == NormMsg::FC)
    {
      if (fc != NULL)
      {
        fc->HandleFcMessage((NormFcMsg&)msg);
      }
      return;
    }
...
{   
  if (fc)
  {
    // We are doing flow control, so make sure we have available space in
    // flow control window before sending packet.
    if (fc->AvailableWindowPkts() == 0)
    {
      return MSG_SEND_WINDOW_BLOCKED;
    }
  }
    ...
    else
    {
        ...
        unsigned int  seqNum = msg.GetSequence();
        if (numBytes == msgSize)
        {
...
//============================================================================
void NormSession::SetWindowFlowControl(bool state)
{
  if (state)
  {
    if (fc == NULL)
    {
      ...
      if (fc == NULL)
      {
        PLOG(PL_WARN, "Error creating new FlowController.\n");
      }
    }
  }
  else
...
//============================================================================
void NormSession::SetSsmSrcPort(UINT16 src_port)
{
  ssm_src_port = src_port;

  if (tx_socket->IsOpen())
  {
    tx_socket->SetSsmSrcPort(ssm_src_port);
  }
