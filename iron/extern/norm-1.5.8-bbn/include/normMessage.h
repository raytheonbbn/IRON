...
class NormFcMsg : public NormMsg
{
  public:
  ...
  // Message building - these messages are never constructed by NORM
  // applications.

  // Message processing.

  NormFcMsg::Flavor GetFlavor() const
  {
    return (Flavor)((UINT8*)buffer)[FLAVOR_OFFSET];
  }
  ...
  enum
  {
    FLAVOR_OFFSET = MSG_OFFSET
  };
}; // end class NormFcMsg

class NormFcWinSizeMsg : public NormFcMsg
{
  public:
  ...
  {
    return ntohs(((UINT16*)buffer)[WIN_SIZE_OFFSET]);
  }

  protected:
  enum
  {
    WIN_SIZE_OFFSET = (FLAVOR_OFFSET + 2) / 2
  };
}; // end class NormFcWinSizeMsg

class NormFcWinUpdateMsg : public NormFcMsg
{
  public:
  ...
  UINT16 GetWindowRcvSeqNum() const
  {
    return ntohs(((UINT16*)buffer)[WIN_RCV_SEQ_NUM_OFFSET]);
  }

  UINT16 GetWindowSentSeqNum() const
  {
    return ntohs(((UINT16*)buffer)[WIN_SENT_SEQ_NUM_OFFSET]);
  }
  ...
  {
    WIN_SIZE_OFFSET         = (FLAVOR_OFFSET + 2) / 2,
    WIN_RCV_SEQ_NUM_OFFSET  = WIN_SIZE_OFFSET + 1,
    WIN_SENT_SEQ_NUM_OFFSET = WIN_RCV_SEQ_NUM_OFFSET + 1
  };
}; // end class NormFcWinUpdateMsg
