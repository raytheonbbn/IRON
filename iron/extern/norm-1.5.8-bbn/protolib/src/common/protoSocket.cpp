...
//============================================================================
bool ProtoSocket::SetSsmSrcPort(UINT16 src_port)
{
  // Create the Berkeley Packet Filter (BPF).
  //
  // The following BPF microcode program compares the first 2 bytes of the UDP
  // header (the source port) to the provided source port of interest. Since
  // we are attaching the BPF to a SOCK_DGRAM socket, the offsets of 0 and 1
  // for the 'load' operations (the 1st and 3rd entries in the code array) are
  // appropriate as described in the following page:
  //
  //   https://stackoverflow.com/questions/24514333/using-bpf-with-sock-dgram-on-linux-machine
  struct sock_filter  code[] =
    {{ 0x30, 0, 0, 0x00000000 },
     { 0x15, 0, 3, (unsigned int)(src_port >> 8) },
     { 0x30, 0, 0, 0x00000001 },
     { 0x15, 0, 1, (unsigned int)(src_port & 0xff) },
     { 0x6, 0, 0, 0x000005dc },
     { 0x6, 0, 0, 0x00000000 }
  };
  ...
  // Attach the packet filter to the socket.
  if (setsockopt(handle, SOL_SOCKET, SO_ATTACH_FILTER, &bpf, sizeof(bpf))
      < 0)
  {
    PLOG(PL_ERROR, "setsockopt SO_ATTACH_FILTER error: %s\n", GetErrorString());
    return false;
  }
