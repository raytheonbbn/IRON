...
proto_register_kupd(void)
{
  ...
  range_convert_str(wmem_epan_scope(), &global_kupd_udp_range, KUPD_PORT_DEFAULTS, 65535);
  kupd_udp_range = range_empty(NULL);
...
proto_reg_handoff_kupd(void)
{
  ...
  kupd_udp_range = range_copy(NULL,global_kupd_udp_range);
