...
static int hf_ironlsa_capacity_incl = -1;
#define H_CAPACITY_FLAG 0x01
static int hf_ironlsa_bin_id        = -1;
static int hf_ironlsa_latency_mean  = -1;
static int hf_ironlsa_latency_stddev= -1;
static int hf_ironlsa_dec_capacity  = -1;
...
/* +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ */
/* | Num neighbors | Num dst bins  |             |C|    Padding    | */
/* +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ */
/* | Latency BinId |      Latency Mean (100us)     |      Latency  | */
/* +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ */
/* | Var (100us)   |   Estimated Capacity (bps)    |    BinId      | */
/* +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ */
/*               . . .                                               */
/* +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ */
/* |      Latency Var (100us)      |   Estimated Capacity (bps)    | */
/* +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ */
                  ...
                  proto_tree *tree, void* data _U_)
{
  ...
  if (tree)
  {
    ...
    static gboolean capacity_in = FALSE;
    proto_item *ti = NULL;
    ...
    if (offset + 2 <= tvb_reported_length(tvb))
    {
      proto_tree_add_item (ironlsa_tree, hf_ironlsa_capacity_incl,
                           tvb, offset, 1, ENC_BIG_ENDIAN);
      capacity_in = (((tvb_get_guint8(tvb, offset)) & 0x1) == 0x1);
      offset += 2; // Skip 2 bytes padding
    }
    for (int binid = 0; binid < num_nbrs; ++binid)
    {
      if (offset + 5 <= tvb_reported_length(tvb))
      {
        proto_tree_add_item (ironlsa_tree, hf_ironlsa_bin_id, tvb, offset, 1, ENC_BIG_ENDIAN);
        offset += 1;
        proto_tree_add_item (ironlsa_tree, hf_ironlsa_latency_mean, tvb, offset, 2, ENC_BIG_ENDIAN);
        offset += 2;
        proto_tree_add_item (ironlsa_tree, hf_ironlsa_latency_stddev, tvb, offset, 2, ENC_BIG_ENDIAN);
        offset += 2;
      }
      if (capacity_in && (offset + 2 <= tvb_reported_length(tvb)))
      {
        guint32 capacity_bps  = 0;
        guint8  d             = 0;
        guint8  e             = tvb_get_guint8(tvb, offset);
        guint8  i             = e & 0xF;

        e                   >>= 4;
        d                     = tvb_get_guint8(tvb, offset + 1);

        capacity_bps          = (i + (d * 4e-3)) * pow(10, e);

        proto_tree_add_uint(ironlsa_tree, hf_ironlsa_dec_capacity, tvb, offset, 2, capacity_bps);
        offset += 2;
      }
    }
...
proto_register_ironlsa(void)
{
  ...
  static hf_register_info hf_ironlsa[] = {
    ...
    { & hf_ironlsa_capacity_incl,
      { "Capacity Included", "ironlsa.capacity_incl",
        FT_BOOLEAN, 8,
        NULL, H_CAPACITY_FLAG,
        NULL, HFILL }
    },
    { & hf_ironlsa_bin_id,
    ...
    { & hf_ironlsa_latency_mean,
      { "Latency Mean", "ironlsa.latency_mean",
        FT_UINT16, BASE_DEC,
        NULL, 0x0,
        NULL, HFILL }
    },
    { & hf_ironlsa_latency_stddev,
      { "Latency StdDev", "ironlsa.latency_stddev",
        FT_UINT16, BASE_DEC,
        NULL, 0x0,
        NULL, HFILL }
    },
    { & hf_ironlsa_dec_capacity,
      { "Capacity", "ironlsa.capacity",
        FT_UINT16, BASE_DEC,
        NULL, 0x0,
        NULL, HFILL }
    },
  ...
  range_convert_str(&global_ironlsa_udp_range, IRONLSA_PORT_DEFAULTS, 65535);
  ironlsa_udp_range = range_empty();
...
proto_reg_handoff_ironlsa(void)
{
  ...
  ironlsa_udp_range = range_copy(global_ironlsa_udp_range);
