...
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
/// |  Type (x10)   |                   Src Address
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
///                 |                 Sequence Number
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
///                 |   Num Groups  |     Group ID 0 (all ucast)    |
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
///      Group ID 0 (all ucast)     |    Num Pairs  |*Dest Bin Id 0 |
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
/// |                   Queue Depth for Bin Id 0                    |
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
/// |                 LS Queue Depth for Bin Id 0                   |
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
/// |*Dest Bin Id 1 |                    Queue Depth for Bin Id 1
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
///  QD for Bin Id 1| ...                                           |
/// +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
/// |                        Group ID 1 (mcast)                     |
...
dissect_qlam (tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void* data _U_)
{
  ...
  if (tree) {
    ...
    if (offset + 1 <= tvb_reported_length(tvb))
    {
      num_groups  = tvb_get_guint8(tvb, offset);
      proto_tree_add_item(qlam_tree, hf_qlam_num_groups, tvb, offset, 1, ENC_BIG_ENDIAN);
      offset += 1;
    }
    ...
    while (num_groups-- > 0)
    {
      if (offset + 4 <= tvb_reported_length(tvb))
      {
        ...
        proto_tree_add_ipv4(qlam_tree, hf_qlam_group_id, tvb, offset, 4, group_id);
        offset += 4;
      }
      ...
      // Process all the (bin, depth) pairs for this group.
      while (num_pairs-- > 0)
      {
        if (offset + 1 <= tvb_reported_length(tvb))
        {
