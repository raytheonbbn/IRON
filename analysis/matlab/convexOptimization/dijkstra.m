...
%==============================================================
% shortestPath: the list of nodes in the shortestPath from source
% to destination;
% totalCost: the total cost of the  shortestPath;
% farthestNode: the farthest node to reach for each node after
...
for i = 1:n
    ...
    farthestPrevHop(i) = i; % used to compute the RTS/CTS
                            % range;
    farthestNextHop(i) = i;
end
...
distance(1:n) = inf;    % it stores the shortest distance between
                        % each node and the source node;
parent(1:n) = 0;
...
for i = 1:(n-1),
    ...
    [t, u] = min(temp);      % it starts from node with the
                             % shortest distance to the
                             % source;
    visited(u) = true;         % mark it as visited;
    for v = 1:n,                % for each neighbors of
                                % node u;
        if ( ( netCostMatrix(u, v) + distance(u)) ...
             < distance(v) )
...
if parent(d) ~= 0   % if there is a shortestPath!
    ...
    while t ~= s
        ...
        if netCostMatrix(t, farthestPrevHop(t)) < netCostMatrix(t, ...
                                                              p)
            farthestPrevHop(t) = p;
        end;
        if netCostMatrix(p, farthestNextHop(p)) < ...
                netCostMatrix(p, t)
            farthestNextHop(p) = t;
        end;
